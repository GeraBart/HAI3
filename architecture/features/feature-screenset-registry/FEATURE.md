...
There is NO public `unmount` method on `ExclusiveMountStrategy`. Eviction happens only as a side effect of `mount`. The mediator rejects unmount actions for domains backed by this strategy at registration time (see `inst-cross-validate-handlers` in `cpt-frontx-flow-screenset-registry-register-domain`).
