...
3. [x] - `p1` - `useDomainExtensions()`, `useRegisteredPackages()` MUST throw if `app.screensetsRegistry` is not present, directing developers to add the `microfrontends()` plugin - `inst-throw-no-registry`
