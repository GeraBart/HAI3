...
- [ ] `p1` - **ID**: `cpt-frontx-dod-mfe-isolation-author-state-lifecycle`
...
**mfe.json as the complete contract**: The enriched `mfe.json` is the sole artifact the host/backend needs per MFE. It contains: human-authored entries, extensions, and schemas, plus plugin-enriched `manifest.metaData`, `manifest.shared[]` (with `chunkPath`/`version`/`unwrapKey`), and per-entry `exposeAssets`. No intermediate artifacts (`mfe.gts-manifest.json`) are produced. The generation script aggregates these `mfe.json` contracts into a public-asset `generated-mfe-manifests.json` that the host (and any nested FrontX app instance) fetches at runtime; the eventual backend API swap returns the same `mfe.json` content via a one-line URL change in the host bootstrap fetch.
