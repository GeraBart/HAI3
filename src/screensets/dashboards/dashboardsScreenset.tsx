...
import { initializeDashboardsEffects } from './effects/dashboardsEffects';

// Import for side effect - register API service
import { DASHBOARDS_DOMAIN } from './api/dashboardsApiService';
import './api/dashboardsApiService';
import { dashboardsMockMap } from './api/mocks';

// Register mock data for API service
apiRegistry.registerMocks(DASHBOARDS_DOMAIN, dashboardsMockMap);
 ...
 */
registerSlice(dashboardsSlice, (dispatch) => {
  initializeDashboardsEffects(dispatch);
});

/**
 * dashboards Screenset Configuration
 */
export const dashboardsScreenset: ScreensetConfig = {
  id: DASHBOARDS_SCREENSET_ID,
  name: 'dashboards',
  ...
  menu: [
    {
      menuItem: {
        id: HOME_SCREEN_ID,
        label: `screenset.${DASHBOARDS_SCREENSET_ID}:menu_items.dashboards.label`,
        icon: HOME_ICON_ID,
      },
 ...
 */
screensetRegistry.register(dashboardsScreenset);
