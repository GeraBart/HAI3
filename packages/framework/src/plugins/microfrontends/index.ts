...
export function microfrontends(config: MicrofrontendsConfig): HAI3Plugin {
  ...
  screensetsRegistry.executeActionsChain = async (chain) => {
    ...
    const beforeByDomain = new Map(
      lifecycleDomains.map((domainId) => [domainId, new Set(screensetsRegistry.getMountedExtensions(domainId))])
    );
    ...
    try {
      ...
      if (lifecycleDomains.length > 0) {
        ...
        for (const domainId of lifecycleDomains) {
          ...
          const after = new Set(screensetsRegistry.getMountedExtensions(domainId));
